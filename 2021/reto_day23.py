...
for i in range(1,11):
    n.append(Node(i,parent=n[i-1]))
#RoomA
n.append(Node(11,parent=n[2]))
n.append(Node(12,parent=n[11]))
#RoomB
n.append(Node(13,parent=n[4]))
n.append(Node(14,parent=n[13]))
#RoomC
n.append(Node(15,parent=n[6]))
n.append(Node(16,parent=n[15]))
#RoomD
n.append(Node(17,parent=n[8]))
n.append(Node(18,parent=n[17]))
...
def p(matrix):
    for m in matrix:
        print(m)
def prHojas(arbol):
    print([x.name[1] for x in arbol.leaves])
def pr2():
    casaini=['.']*11+['A','B','C','D','C','A','D','B']
...
#         historico.append(m)
soluciones=[]
arbol=Node((casaini,[0,0]))
cp=posiblesMov()
casas=[]
k=0
casas.append(casa)
for p in cp[1]:
    Node((cp[0][:],p),parent=arbol)
fin=False
while(True):
    print("Ronda["+str(k)+"]")
    if k>=10:

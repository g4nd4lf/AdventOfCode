...
def ordena(estados):
    est=numpy.array([list(x[0][0]+x[0][1]+(x[1],)) for x in estados], dtype=numpy.uint32)
    for i in range(7, -1, -1):
        ...
        for est,g in oldEstados.items():
            ...
            for i in range(len(robotsCreables)):
                if robots[i]>=tope(robots,t)[i]:
                    robotsCreables[i]=0
            orde=list(permutations([0,1,2]))
            ordenes=[(3,)+x for x in orde]
            #ordenes=list(permutations([0,1,2]))

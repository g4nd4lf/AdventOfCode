...
for b1 in blueprints:
    ...
    while t<25:
       ...
       # for e in estados:
        ...
        for e in oldEstados:
            ...
            for orden in ordenes:
                ...
                for o in orden:
                    if resources[o]>maxis[o]:
                        continue
                    elif robotsCreables[o]: #and enoughtResources(o,op[o],newe,b1)
                        res,rob=creaRobot(o,res,rob,b1)
                        alo=0
                for i in range(len(res)):
                    res[i]+=robots[i]
                nuevoEstado=(tuple(res),tuple(rob))
                if nuevoEstado[0] not in estados:
                    estados[nuevoEstado[0]]=nuevoEstado[1]
                else:
                    val1=sum([a*b for a,b in zip((nuevoEstado[1]),(1,10,100,10000))])
                    val2=sum([a*b for a,b in zip(estados[nuevoEstado[0]],(1,10,100,10000))])
                    if (val1>val2):
            ...
            #                     newe=creaRobot(o,numrobots[o],newe,b1)
    ...
    print(max([v[3] for (k,v) in estados.items()]))
print(geodas)
